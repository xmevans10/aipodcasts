...
struct TranscriptView: View {
    ...
    private static let bodyFont = Font.custom("Charter", size: 21, relativeTo: .body)
        ...
        ScrollViewReader { proxy in
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    Eyebrow(text: "READ ALONG · \(story.host.name.uppercased())")
                    Text(story.title).font(Self.titleFont).foregroundStyle(ScienceBreak.ink)
    ...
    private func controls(scrollBackTo active: Int, proxy: ScrollViewProxy) -> some View {
        VStack(spacing: 10) {
            if !following && isCurrent && player.playing {
                ...
                } label: { Label("Follow along", systemImage: "arrow.down.to.line").font(.caption.weight(.semibold)).padding(.horizontal, 14).frame(height: 34).background(ScienceBreak.acid, in: Capsule()) }
            ...
            HStack(spacing: 18) {
                ...
                Button { isCurrent ? player.toggle() : player.play(story) } label: {
                    Image(systemName: isCurrent && player.playing ? "pause.fill" : "play.fill").font(.title3)
                        .frame(width: 54, height: 54).background(ScienceBreak.acid, in: Circle()).foregroundStyle(ScienceBreak.ink)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ProgressView(value: isCurrent ? min(player.position, player.duration) : 0, total: max(player.duration, 1)).tint(ScienceBreak.acid)
                    Text(isCurrent ? "\(clock(player.position)) / \(clock(player.duration))" : "Tap play or any paragraph")
                        .font(.caption2.monospacedDigit()).foregroundStyle(ScienceBreak.paper.opacity(0.75))
                }
            }
            .foregroundStyle(ScienceBreak.paper)
            .padding(.horizontal, 16).padding(.vertical, 10)
            .background(ScienceBreak.ink, in: RoundedRectangle(cornerRadius: 26))
        }
    ...
    private func attributed(_ paragraph: TranscriptParagraph, active: Bool) -> AttributedString {
        ...
        for (index, word) in paragraph.words.enumerated() {
            ...
            piece.foregroundColor = spoken ? ScienceBreak.ink : ScienceBreak.muted
            if active && index == spokenCount - 1 && isCurrent { piece.backgroundColor = ScienceBreak.acid }
            result += piece
