"""Science Break's local editorial pipeline. Python 3.11+, standard library only.
    ...
    class NoRedirect(urllib.request.HTTPRedirectHandler):
        def redirect_request(self, req, fp, code, msg, hdrs, newurl):
            ...
            if (req.data is None and old.hostname == "journals.plos.org"
                ...
                return urllib.request.Request(newurl, headers={"User-Agent": "Science BreakResearch/0.1"})
            return None
    data = json.dumps(payload).encode() if payload is not None else None
    req = urllib.request.Request(url, data=data, headers={"User-Agent": "Science BreakResearch/0.1", **(headers or {})})
    ...
    for r in db.execute("SELECT * FROM stories WHERE state='published' ORDER BY created DESC, id"):
        ...
        result.append({"id": r["id"], "title": draft["title"], "dek": draft["dek"],
            ...
            "sources": [{"title": source["title"], "url": source["url"],
                         "attribution": source["attribution"] + ". Adapted by Science Break; changes made.",
